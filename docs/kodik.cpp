// Replace file with exact kod.txt implementation (cleaned and adjusted)
...
int main(int argc, char **argv)
{
    ...
    const char *input_name = nullptr; // unset -> use built-in fallback
    std::string function_name = "wavy_network";
    bool metrics_each_collapse = false;

    // Parse argv (simple, order-independent)
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            if (i + 1 < argc)
                angle_treshold = std::stof(argv[++i]);
        }
        else if (s == "-d" || s == "--divisor")
        {
            if (i + 1 < argc)
                divisor_median = std::stof(argv[++i]);
        }
        else if (s == "--metrics-each-collapse")
        {
            metrics_each_collapse = true;
        }
        else if (s == "-h" || s == "--help")
        {
            std::cout << "Usage: Isosurfaces [input-mesh] [options]\n";
            std::cout << "  -f, --function <name>   implicit surface to project onto\n";
            std::cout << "  -o, --output <file>     output mesh (default: output.obj)\n";
            std::cout << "  -n, --iterations <N>    number of remeshing passes (default: 6)\n";
            std::cout << "  -a, --angle <degrees>   collapse triangles with an angle below this\n";
            std::cout << "  -d, --divisor <x>       collapse triangles smaller than median area / x\n";
            std::cout << "      --no-gui            do not open a window\n";
            std::cout << "      --metrics-each-collapse  print metrics after every collapse (slow)\n";
            return 0;
        }
        else
        {
            // positional argument -> input filename (first one wins)
            if (!input_name)
                input_name = argv[i];
        }
    }

    // Create viewer only if GUI requested
    iso::MyViewer *window = nullptr;
    if (gui_enabled)
        window = new iso::MyViewer("View", 1024, 768);

    SurfaceMesh mymesh;

    // If no input filename provided, use the repository default
    if (!input_name)
        input_name = "assets/mesh_catalog/wavy_network.obj";

    // Diagnostic: print chosen input and test file existence
    std::cerr << "Using input file: '" << input_name << "'\n";
    {
        FILE *f = fopen(input_name, "r");
        if (f)
        {
            fclose(f);
            std::cerr << "File exists and is readable: '" << input_name << "'\n";
        }
        else
        {
            std::cerr << "File does not exist or is not readable: '" << input_name << "'\n";
        }
    }
    ...
    {
        std::cerr << "Warning: pmp::read failed for '" << input_name << "': " << e.what() << std::endl;
        std::cerr << "Trying fallback 'assets/sphere.obj'...\n";
        try
        {
            pmp::read(mymesh, "assets/sphere.obj");
        }
        catch (const pmp::IOException &e2)
        {
            std::cerr << "Error: failed to open fallback 'assets/sphere.obj': " << e2.what() << std::endl;
            delete window;
            return 1;
        }
    }
